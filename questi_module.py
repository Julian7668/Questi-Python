...
class Questi:
    ...
    def text(
        ...
        Args:
            ...
            validate_user (Union[float, Callable[[str], bool]]): Tipo de validación:
                - 1.0: Validar que no esté vacío (string)
                - 2.0: Validar entero dentro del rango
                - 3.0: Validar flotante dentro del rango
        ...
        Example:
            >>> nombre = questi.text("¿Tu nombre?")  # string
            >>> edad = questi.text("¿Tu edad?", validate_user=2.0, inicio_rango=0, fin_rango=120)  # entero
            >>> peso = questi.text("¿Tu peso?", validate_user=3.0, inicio_rango=0.0, fin_rango=300.0)  # flotante
        """
        ...
        def _questi_text() -> Optional[Union[str, int, float]]:
            ...
            validaciones: Dict[float, Callable[[str], bool]] = {
                ...
                2.0: lambda x: Validates.validate_int(
                    x.strip(), inicio_rango, fin_rango
                ),
                3.0: lambda x: Validates.validate_float(
                    x.strip(), inicio_rango, fin_rango
                ),
            }
            ...
            if isinstance(validate_user, float) and validate_user in validaciones:
                if resultado := questionary.text(
                    ...
                    if validate_user == 2.0:
                        what_return = int
                    elif validate_user == 3.0:
                        what_return = float
